window.cardsFocus = [
  ...
  // ===== Apex OOP: Virtual Classes & Inheritance =====
  {
    id: "Q21",
    question: "Which keyword must be applied to a class or method so that a subclass is allowed to override it?",
    code: "public virtual class Payment {\n    public virtual Decimal getFee() {\n        return 0;\n    }\n}",
    options: [
      { letter: "A", text: "static" },
      { letter: "B", text: "virtual (or abstract)" },
      { letter: "C", text: "final" },
      { letter: "D", text: "global" },
    ],
    answer: "B",
    explanation: "A method can only be overridden if its class is declared virtual or abstract and the method itself is marked virtual or abstract. By default Apex methods are final and cannot be overridden.",
  },
  {
    id: "Q22",
    question: "Given this code, what does new CreditCard().getFee() return?",
    code: "public virtual class Payment {\n    public virtual Decimal getFee() { return 1; }\n}\n\npublic class CreditCard extends Payment {\n    public override Decimal getFee() { return 5; }\n}",
    options: [
      { letter: "A", text: "1" },
      { letter: "B", text: "5" },
      { letter: "C", text: "6" },
      { letter: "D", text: "Compile error - cannot extend a virtual class" },
    ],
    answer: "B",
    explanation: "CreditCard overrides getFee(), so the subclass implementation (return 5) is executed. The 'override' keyword is required when replacing a virtual/abstract method.",
  },
  {
    id: "Q23",
    question: "What is required when a class extends an abstract class that contains an abstract method?",
    code: "public abstract class Shape {\n    public abstract Decimal area();\n}\n\npublic class Circle extends Shape {\n    // ???\n}",
    options: [
      { letter: "A", text: "Nothing; abstract methods are optional to implement" },
      { letter: "B", text: "The subclass must provide an implementation of every abstract method (or be abstract itself)" },
      { letter: "C", text: "The subclass must also be declared abstract" },
      { letter: "D", text: "The subclass must use the 'implements' keyword instead of 'extends'" },
    ],
    answer: "B",
    explanation: "An abstract method has no body and MUST be implemented by the first concrete subclass. Circle must define 'public override Decimal area() { ... }' or be declared abstract itself.",
  },
  {
    id: "Q24",
    question: "How does a subclass call the constructor of its parent (virtual) class?",
    code: "public virtual class Account_ {\n    public String name;\n    public Account_(String n) { this.name = n; }\n}\n\npublic class VIP extends Account_ {\n    public VIP(String n) {\n        super(n);\n    }\n}",
    options: [
      { letter: "A", text: "this(n)" },
      { letter: "B", text: "super(n)" },
      { letter: "C", text: "parent(n)" },
      { letter: "D", text: "Account_.constructor(n)" },
    ],
    answer: "B",
    explanation: "The super(...) keyword invokes the parent class constructor and must be the first statement in the subclass constructor. Use this(...) to call another constructor in the same class.",
  },
  {
    id: "Q25",
    question: "What is the difference between 'extends' and 'implements' in Apex?",
    code: "public class CreditCard extends Payment implements Refundable {\n    // inherits Payment, fulfills Refundable contract\n}",
    options: [
      { letter: "A", text: "They are interchangeable keywords" },
      { letter: "B", text: "'extends' inherits from one (virtual/abstract) class; 'implements' fulfills one or more interfaces" },
      { letter: "C", text: "'extends' is for interfaces; 'implements' is for classes" },
      { letter: "D", text: "A class can extend multiple classes but implement only one interface" },
    ],
    answer: "B",
    explanation: "Apex supports single class inheritance via 'extends' (one superclass) and multiple interface implementation via 'implements'. Interfaces define a contract with no method bodies.",
  },
  {
    id: "Q26",
    question: "A developer wants to reuse the parent's logic inside an overriding method and then add to it. How is the parent's version invoked?",
    code: "public class CreditCard extends Payment {\n    public override Decimal getFee() {\n        return super.getFee() + 2;\n    }\n}",
    options: [
      { letter: "A", text: "Payment.getFee()" },
      { letter: "B", text: "super.getFee()" },
      { letter: "C", text: "this.getFee()" },
      { letter: "D", text: "base.getFee()" },
    ],
    answer: "B",
    explanation: "super.getFee() calls the parent class's implementation from within the overriding method. Calling this.getFee() would recurse into the override itself.",
  },
];
